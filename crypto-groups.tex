...
\begin{defn}
  Let $G$ be a group and $S$ a set.  An \emph{action} of $G$ on $S$ is a
  function
  \[ \cdot:G\times S\to S\]
  such that
  \begin{enumerate}
...
action.  This particular action is famous and is known as \emph{left
  translation}.
\end{exam}
...
%The key stream is also determined by  the \emph{initial
...
\begin{defn}
  ...
  such that
  \[ f(a*b)  = f(a)*'f(b)\qquad\text{for all $a,b\in H$}\]
  is called a \emph{homomorphism from $H$ to $K$}.
\end{defn}
...
\begin{exam}
  ...
  is a homomorphism called the \emph{reduction map}.  It is onto, but
  not 1-1.
\end{exam}
\begin{exam}
  If $H$ and $K$ are any two groups, the constant function $f:H\to K$
  given by $f(a)=e_K$ for all $a\in H$ is a homomorphism, called the
  \emph{trivial map}.  If $H$ and $K$ have more than one element each,

...
\renewcommand{\Im}{\textrm{Im}} % don't need imaginary part
...
These notes were written for the beginning of MAT 448, {\em Cryptography
...
We consider another instance of something being {\em well-defined}.
...
\end{proof}
\begin{exam}
  To see an example where something is {\em not} well-defined,
...
\begin{defn}
A {\em binary operation} $*$ on a set $S$ is a function
\[ *:S\times S \to S.\]
...
\begin{itemize}
\item  $*$ is {\em associative} if for all
  $a,b,c\in S$,
  \[ a*(b*c)=(a*b)*c.\]
\item $*$ is {\em commutative} if for all $a,b\in S$,
  \[ a*b = b*a.\]
\item $*$ has {\em identity} $e$ if $e\in S$ and for all $a\in S$,
  \[ a*e=e*a=a.\]
\end{itemize} 
\end{defn}
...
\begin{defn}
  ...
  If $a\in S$, we say that $b\in S$ is an {\em inverse} of $a$ if
  \[ a*b=b*a=e.\]
\end{defn}
...
\begin{defn}
  A {\em group} is a set $G$ with a binary operation $*$ such that 
\begin{enumerate}
\item $*$ is associative
...
\begin{defn}
  A group $G$ is {\em abelian} if the operation is commutative.
\end{defn}
...
When this happens, we say that $H$ is {\em closed under $*$}.

\begin{defn}
  If $G$ is a group with operation $*$ and $H\seq G$, then we say that
  $H$ is a {\em subgroup} of $G$ if
  \begin{enumerate}
...
\begin{prob}
  ...
  $H\seq G$, then $H$ is a {\em subgroup} of $G$ if the binary
...
\begin{defn}
  Let $G$ be a group and $S$ a set.  An {\em action} of $G$ on $S$ is a
  function
  \[ \cdot:G\times S\to S\]
  such that
  \begin{enumerate}
...
action.  This particular action is famous and is known as {\em left
  translation}.
\end{exam}
...
For the {\em affine cipher}, we let $\calA=\calP=\calC=\Zn$.
Our group is
the {\em affine group modulo $n$}:
...
For the {\em Hill cipher}, we let $\calA=\Zn$ and pick
...
Let $n\in\Z^+$.  A {\em permutation} on a set $A$ is a bijective
function $f:A\to A$.  The {\em symmetric group} on $A$ is
...
\subsection{LFSRs}
\begin{comment}
\textbf{This section is not finished.  It needs an explanation of
how to generically make a stream cipher from a group action,
what an LFSR is, and how the general process produces an LFSR.
Then again, it may be deleted at a later time.}
\end{comment}

A {\em linear feedback shift register}, or LFSR, is a type of stream
...
%We can picture an $n$-stage LFSR as follows
%\[\begin{CD}
%@| @. @| @. \\
%\boxed{x_k} @>>> \boxed{x_{k+1}} @>>> \cdots @>>> \boxed{x_{k+n-1}} @>>> x_k \\
%\end{CD}
%\]

%The key stream is also determined by  the {\em initial
% loading} of the LFSR, i.e., the first $n$ bits in the sequence.
...
\begin{defn}
  ...
  operation $*'$, then an {\em isomorphism from $G_1$ to $G_2$} is a
  bijective function
  \[ f:G_1\to G_2\]
  such that
  \[ f(a*b)  = f(a)*'f(b)\qquad\text{for all $a,b\in H$.}\]
  We then say that $G_1$ and $G_2$ are {\em isomorphic} and write
  $G_1\cong G_2$.
\end{defn}
...
\begin{defn}
  ...
  such that
  \[ f(a*b)  = f(a)*'f(b)\qquad\text{for all $a,b\in H$}\]
  is called a {\em homomorphism from $H$ to $K$}.
\end{defn}
...
\begin{defn}
 If $f:G\to K$ is a group homomorphism, the {\em kernel} of $f$ is
...
\begin{defn}
  Let $G$ be a group and $g\in G$.  Then the {\em cyclic subgroup
    generated by $g$} is
...
Here we introduce two closely related notions for elements of a group, {\em order}
and {\em torsion}.

\begin{defn}
If $G$ is a group, $g\in G$, and $n\in\Z^+$, then we say that {\em $g$ is $n$-torsion}
if $g^n=e$.
\end{defn}
...
\begin{defn}
  ...
  \begin{enumerate}
  \item If $g^n\neq e$ for all $n\in\Z^+$, we say that $g$ has {\em infinite order}.
  \item Otherwise, the {\em order of $g$} is the smallest positive integer $n$ such that
  $g^n=e$.
  \end{enumerate}
...
\begin{defn}
  ...
  This is refered to as the {\em $m$-torsion subgroup} of $G$.
\end{defn}
...
\subsection{The most important group for crytography}
...
\begin{defn}
  A set $R$ with two binary operations $+$ and $\cdot$ is a {\em ring}
  if 
  \begin{enumerate}
...
\begin{defn}
  A ring $R$ is a {\em ring with one} if $R$ has an identity for
  multiplication.
\end{defn}
Most of the our rings satisfy the next condition as well.
\begin{defn}
  A ring is a {\em commutative ring} if multiplication is
  commutative.
\end{defn}
...
\begin{exam}
  ...
  multiplication).  Note, it is {\em not} a commutative ring if $n>1$.
...
\begin{exam}
  ...
  called the {\em trivial ring}, and has the peculiar property that
  $0_R=1_R$.
\end{exam}
...
multiplication is called a {\em unit}.  
\end{defn}
...
\begin{defn}
  An {\em endomorphism} of an abelian group $A$ is a homomorphism
...
\begin{defn}
  A {\em field} $F$ is a commutative ring with one with $1_F\neq 0_F$,
...
\begin{defn}
  ...
  additive group, we say $R$ has {\em characteristic zero}.  Otherwise, the
  {\em characteristic of $R$} is the order of $1_R$ under addition.  We
...
irreducible.  The result is then called a {\em curve}.
...
work over a field $K$, and define {\em affine $n$-space} to be $K^n$
...
X^2+3Y^2=Z^2.
\end{equation}
The end result is a {\em homogeneous equation} in that every term has
total degree $2$ (the {\em total degree} of a monomial is the sum of
...
\begin{defn}
  ...
  {\em left coset of $a$}, denoted $aH$ is the set
  \[ aH=\{ah\mid h\in H\}.\]
\end{defn}
...
constant.  So, we normally deal with {\em monic} polynomials, i.e.,
...
over $K$ with $\alpha$ as a root), and is called the {\em monic
...
If a {\em finite field} is simply a field $K$ which is a finite set.
...
\begin{defn}\label{divides}
  If $a,b\in\Z$, then we say that $a$ {\em divides} $b$
